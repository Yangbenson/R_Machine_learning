...
mnist_raw <- read.csv("~/Desktop/Assigments/2023_DM/file/mnist_test (1).csv")
...
### SECTION 2-----------
mnist_raw[, "zero"] = as.numeric(mnist_raw[,"X7"] == 0)
mnist_raw[, "one"] = as.numeric(mnist_raw[,"X7"] == 1)
mnist_raw[, "two"] = as.numeric(mnist_raw[,"X7"] == 2)
mnist_raw[, "three"] = as.numeric(mnist_raw[,"X7"] == 3)
mnist_raw[, "four"] = as.numeric(mnist_raw[,"X7"] == 4)
mnist_raw[, "five"] = as.numeric(mnist_raw[,"X7"] == 5)
mnist_raw[, "six"] = as.numeric(mnist_raw[,"X7"] == 6)
mnist_raw[, "seven"] = as.numeric(mnist_raw[,"X7"] == 7)
mnist_raw[, "eight"] = as.numeric(mnist_raw[,"X7"] == 8)
mnist_raw[, "nine"] = as.numeric(mnist_raw[,"X7"] == 9)
...
inputs = ncol(mnist_raw_train[,-c(785:795)])
...
for (i in 1:784) {
  ...
  target_lm = paste(target_lm, n[i],sep="+")
  
}
f = target_lm

# stepwise equation
glm_df_train = mnist_raw[1:1000,-c(785:794)]
glm_df_test = mnist_raw[1001:2000,-c(785:794)]

library(class)

k = 10 
accuracy = 0
for (i in 1:nrow(glm_df_test)) {
  pred <- knn(glm_df_train[,-785], glm_df_test[i, -785], class, k)
  if (pred == glm_df_test[i,785]) {
    accuracy = accuracy + 1
  }
}
accuracy <- accuracy/nrow(glm_df_test[,-785])
